        ...
        @else
            <div class="grid grid-cols-1 @lg:grid-cols-2 @3xl:grid-cols-3 @6xl:grid-cols-4 gap-2">
                ...
                @foreach ($userRequestCounts as $userRequestCount)
                    <x-pulse-boosted::user-card wire:key="{{ $userRequestCount->key }}" :user="$userRequestCount->user" wire:click="$dispatch('open-person', { id: {{ \Illuminate\Support\Js::from((string) $userRequestCount->key) }} })" class="cursor-pointer hover:ring-1 hover:ring-accent-500/40" title="Open this user">
                        <x-slot:stats>
                            @if ($sampleRate < 1)
