...
{{-- Every log line and exception, one entry at a time, newest first. --}}
@php
...
@endphp
    ...
    <x-pulse-boosted::scroll :expand="$expand" wire:poll.30s.visible="">
        ...
        @else
            ...
            @if ($entries->count() >= $perPage || $page > 1)
                <div class="flex items-center justify-end gap-2 mt-3 text-xs text-gray-500 dark:text-gray-400">
                    ...
                    <button type="button" wire:click="$set('page', {{ $page + 1 }})" @disabled($entries->count() < $perPage)
